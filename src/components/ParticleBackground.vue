...
onMounted(() => {
  ...
  const init = () => {
    ...
    for (let i = 0; i < numParticles; i++) {
      ...
      particles.push({
        ...
        y: (Math.random() - 0.5) * (Math.random() * 120),
  ...
  const draw = () => {
    ...
    const tiltX = Math.PI / 2.5 - mouseY * 0.15;
    const tiltY = Math.PI / 12 + mouseX * 0.15;
    ...
    for (let i = 0; i < pointsToDraw.length; i++) {
      ...
      if (p1.originalRadius > 500) continue;
...
<template>
  <canvas ref="canvasRef" class="fixed inset-0 pointer-events-none z-0 opacity-80" />
</template>
